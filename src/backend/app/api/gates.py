...
async def _get_decidable_gate(session: AsyncSession, gate_id: uuid.UUID, user: User) -> Gate:
    """课题主人才可见/可审批（见 gates.can_access_project）；够不着的人一律 404（不泄露存在性）。"""
...
async def _decide(
    *,
    ...
    # 投稿联动（M5-B）：paper_submission 批准 → submitted；驳回 → 回退 compiled
    manuscript = await manuscripts_service.decide_submission_from_gate(
        session, gate, approved=approved
    )
