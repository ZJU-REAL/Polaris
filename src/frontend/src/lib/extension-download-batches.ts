...
/** 后端 DOWNLOADABLE_LIBRARY_STATUSES：未晋级的候选不建资产，推送必被 404 拒绝。 */
export const EXTENSION_SENDABLE_STATUSES = ['scored', 'fetched', 'compiled', 'included'] as const;

export function canSendToExtension(status: string | null | undefined): boolean {
  return EXTENSION_SENDABLE_STATUSES.includes(status as never);
}
